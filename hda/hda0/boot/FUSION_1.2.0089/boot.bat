...
:CHECK -------------------------------------------------------------------------
IF NOT EXIST "etc\conf\fusion\boot.ap_conf" GOTO ERROR_BOOT_0B-0006-001
IF NOT EXIST "etc\conf\ufe\kernel.ap_conf" GOTO ERROR_BOOT_0B-0007-002

:BOOT --------------------------------------------------------------------------
:: INSTALLATION ------------------------------------------------------
FOR /F "eol=[ tokens=1,2 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==INSTALL_DRIVE SET INSTALL_DRIVE=%%b
FOR /F "eol=[ tokens=1,2 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==INSTALL_PAR SET INSTALL_PAR=%%b>nul

:: WINDOW_CONFIG -----------------------------------------------------
FOR /F "eol=[ tokens=1,2,3 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==SIZE MODE CON:COLS=%%b lines=%%c
FOR /F "eol=[ tokens=1,2,3 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==CODE_PAGE MODE CON CP SELECT=%%b lines=%%c>nul
FOR /F "eol=[ tokens=1,2 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==TITLE TITLE %%b
FOR /F "eol=[ tokens=1,2 delims==" %%a in (etc\conf\fusion\boot.ap_conf) do IF %%a==COLOR COLOR %%b
