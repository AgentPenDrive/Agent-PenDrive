...
:: IF NOT EXIST "%1.vme_hdd" GOTO ERROR_VME-BOOT_HDD_0A-0006-001
...
CLS
FOR /F "tokens=1,2 delims==" %%a in (includes\langs\%VME_LANG%.vme_lng) do IF %%a==ERROR_VME-BOOT_HDD_0A-0006-001 (
...
CLS
FOR /F "tokens=1,2 delims==" %%a in (includes\langs\%VME_LANG%.vme_lng) do IF %%a==ERROR_VME-BOOT_HDD_0A-0010-001 (
  IF EXIST "includes\cmds\wbat250\WBAT.COM" CALL "includes\cmds\wbat250\WBAT" BOX %%b OK
  IF NOT EXIST "includes\cmds\wbat250\WBAT.COM" ECHO %%b & PAUSE
)
GOTO END
